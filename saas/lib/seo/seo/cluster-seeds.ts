 ...
 * Generates ~1,000 article cluster seeds for the pSEO system.
 */
...
function generateLongTailClusters(count: number): ClusterSeed[] {
  ...
  const variations = pickN(longTailTopics, Math.min(count, longTailTopics.length));
...
function generateAllClusters(): ClusterSeed[] {
  const allClusters: ClusterSeed[] = [];

  allClusters.push(...generateUseCaseClusters(CLUSTER_CONFIGS.use_case.targetCount));
  allClusters.push(...generateProductivityTipClusters(CLUSTER_CONFIGS.productivity_tip.targetCount));
  allClusters.push(...generateFeatureGuideClusters(CLUSTER_CONFIGS.feature_guide.targetCount));
  allClusters.push(...generateComparisonClusters(CLUSTER_CONFIGS.comparison.targetCount));
  allClusters.push(...generateIntegrationClusters(CLUSTER_CONFIGS.integration.targetCount));
  allClusters.push(...generateIndustryClusters(CLUSTER_CONFIGS.industry.targetCount));
  allClusters.push(...generateRoleClusters(CLUSTER_CONFIGS.role.targetCount));
  allClusters.push(...generateProblemSolutionClusters(CLUSTER_CONFIGS.problem_solution.targetCount));
  allClusters.push(...generateBestPracticeClusters(CLUSTER_CONFIGS.best_practice.targetCount));
  allClusters.push(...generateSeasonalClusters(CLUSTER_CONFIGS.seasonal.targetCount));
  allClusters.push(...generateTemplateClusters(CLUSTER_CONFIGS.template.targetCount));
  allClusters.push(...generateLongTailClusters(CLUSTER_CONFIGS.long_tail.targetCount));
...
export async function seedArticleClusters(options: { force?: boolean; append?: boolean } = {}): Promise<SeedResult> {
  ...
  if (existingCount > 0 && !force && !append) {
    return {
      ...
      existingCount,
      summary: {},
      error: `Found ${existingCount} existing clusters. Use force=true to delete and recreate, or append=true to add more.`,
    };
  }
  ...
  // Generate clusters
  const allClusters = generateAllClusters();
  ...
  // Get summary
  const summaryData = await prisma.articleCluster.groupBy({
    by: ["clusterType"],
    _count: true,
  });
  ...
  for (const item of summaryData) {
    summary[item.clusterType] = item._count;
  }
  ...
  return {
    ...
    existingCount: force ? 0 : existingCount,
    summary,
  };
}
